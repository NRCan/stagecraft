...
def parse_tx_row(row):
    ...
    agency_matches_department = \
        agency['abbr'] == dept['abbr'] or agency['name'] == dept['name']

    if agency_matches_department:
        agency = None
...
def parse_rows(worksheet, parse_fn, slug_col):
    return {r[slug_col]: parse_fn(r) for r in worksheet.get_all_values()[1:]}
